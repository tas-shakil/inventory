...
class InvoiceController extends Controller
{
    public function view(){
        ...
        return view('Invoice/view-invoice',compact('allData'));
    }
    ...
    public function store(Request $request){
        if($request->category_id == null){
            ...
            if($request->paid_amount > $request->estimated_amount){
                return redirect()->back()->with('error','Sorry! Paid amount is grater than total price');
            }else{
                $invoice = new Invoice();
   ...
   return redirect()->route('invoice.view')->with('success','Data save successfully');
     ...
     public function delete($id){
        ...
        return redirect()->route('invoice.pending.list')->with('success','Delete Success');
          
      }

      public function pendingList(){
        $allData= Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','0')->get();
        return view('Invoice/view-pending-invoice',compact('allData'));
      }

      public function invoiceApproved($id){
          $invoice = Invoice::with(['invoice_details'])->find($id);
          $payment = Payment::where('invoice_id', $invoice->id)->first();
          return view('Invoice/invoice-approved',compact('invoice','payment'));  
      }
      ...
      public function approvalStore(Request $request, $id){
          foreach($request->selling_qty as $key =>$val){
              ...
              if($product->quantity < $request->selling_qty[$key]){
                  return redirect()->back()->with('error','Sorry! one of prduct stock out');
              }
          }
      ...
      public function invoiceList(){
        $allData= Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','1')->get();
        return view('Invoice/invoice-list',compact('allData'));
         
      }

      public function invoicePrint($id){
        $data['invoice'] = Invoice::with(['invoice_details'])->find($id);
        $data['payment'] = Payment::where('invoice_id', $data['invoice']->id)->first();
